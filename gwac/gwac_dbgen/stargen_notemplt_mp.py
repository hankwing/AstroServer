...
def genOneSimuCatalog(catano, abnormpoint, newcatano, ccd, destdir, templatefile, rac, decc, sqd, wcs_transtool, origintime, pixscal=11.7, sizes=[4096,4096], zoneheig=10.):
    """
    ...
    catano=newcatano
    simcatafilename = "%sRA%03d_DEC%d_sqd%d-ccd%s-%s-%s.cat"  %(destdir, rac, decc, sqd, ccd, catano, str(datetime.datetime.now()).replace(" ", "").replace(":","-"))
...
def getOneNightCatalog(catano, abnormpoint, newcatano, ccd, destdir, templatefile, rac, decc, sqd, pixscal, sizes, zoneheig, origintime):
    """
    ...
    """
    wcs=genwcs(rac,decc,pixscal,sizes)
    genOneSimuCatalog(catano, abnormpoint, newcatano, ccd, destdir, templatefile, rac, decc, sqd, wcs, origintime, pixscal, sizes, zoneheig)
...
def star_generator(**workparams):
    ...
    pool.map(functools.partial(getOneNightCatalog, abnormpoint=workparams['abnormpoint'], newcatano=workparams['newcatano'], ccd=workparams['ccdno'], destdir=workparams['destdir'], templatefile=workparams['templatefile'], rac=workparams['rac'], decc=workparams['decc'], sqd=workparams['sqd'], pixscal=workparams['pixscal'], sizes=workparams['sizes'], zoneheig=workparams['zoneheight'], origintime=workparams['n1cata']), range(workparams['n1cata'], workparams['n2cata']+1,15)) 
