...
writeLogRound=1 #write master log per 5 * $sleeptime (s)
...
do
   ccdno=$(($ccdno+1))
   {
    if [ "$line" == "$abhost" ]
      then
           ssh -f -n $username@$line "cd gwac/gwac_dbgen_cluster/; nohup ./sendStaDataPerNode_collect.sh $mastername $ccdno $abnum $redisIP $templateTable $sourceFilePath $threadNumber $getTemplateFlag >/dev/null 2>&1 &"
#echo "$mastername $ccdno $abnum $redisIP $templateTable $sourceFilePath $threadNumber $getTemplateFlag"
#           echo $abhost $abnum
      else 
           ssh -f -n $username@$line "cd gwac/gwac_dbgen_cluster/; nohup ./sendStaDataPerNode_collect.sh $mastername $ccdno $(($RANDOM % 10)) $redisIP $templateTable $sourceFilePath $threadNumber $getTemplateFlag >/dev/null 2>&1 &"
#echo "$mastername $ccdno $abnum $redisIP $templateTable $sourceFilePath $threadNumber $getTemplateFlag" 
#          echo "$abnum  ok"
     fi
